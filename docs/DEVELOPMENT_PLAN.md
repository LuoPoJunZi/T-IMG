...
| DEV-016 | 移除上传页显式退出按钮 | 已完成 | 两个上传页面、结构测试和文档 | 页面无退出控件、退出接口保留、完整回归通过 |
| DEV-017+ | 其他正式业务功能 | 已暂停 | 待分析 | 等待项目所有者确认 |
...
DEV-001 至 DEV-016 均已完成；其他示例业务功能保持暂停。当前分支为 `main`，仅跟踪个人公开 `origin`；认证简化、博客和 WAF 教程已由提交 `c55c3c3` 推送，CI 审计修复已由提交 `690fc7a` 推送并通过 GitHub Actions，后续提交与推送状态以 Git 历史为准，生产 Cloudflare 配置仍由项目所有者管理。
