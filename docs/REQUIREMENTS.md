...
- 需求名称：移除上传页显式退出按钮
- 需求描述：从主上传页和 Markdown 上传页移除右下角“退出上传”按钮及其专用样式，保持上传界面简洁。
- 用户价值：去除不必要的固定悬浮控件，避免遮挡页面内容，同时不改变上传认证的安全边界。
- 涉及页面：`index.html`、`markdown-upload.html`；不修改登录页、上传逻辑、公开文件页或后台管理页。
- 接口与会话要求：保留 `POST /api/upload-auth/logout` 及 Cookie 清除能力；会话仍按 `UPLOAD_SESSION_MAX_AGE` 到期，用户也可通过清除站点 Cookie 主动结束会话。
- 兼容要求：原有上传、拖拽、进度、结果展示、页面与接口双重认证、公开 `/i/*`、`/file/*` 和后台管理行为不变。
- 验收标准：两个上传页面均不包含退出按钮、对应表单或专用样式；退出接口回归测试继续通过；完整测试通过。
- 优先级：中
- 状态：已完成
...
- DEV-009 不重设计原有上传、拖拽、进度或结果展示交互；DEV-016 后仅保留认证入口和后端退出接口，不再显示退出按钮。
