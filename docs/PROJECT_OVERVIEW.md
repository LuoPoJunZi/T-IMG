...
T-IMG 是一个静态前端与 Cloudflare Pages Functions 组合的文件托管项目。当前版本通过 Telegram Bot API 上传和存储文件，并通过同一 Pages 域名提供访问链接；上传页面和上传接口由独立密码会话保护；绑定 Cloudflare KV 后可记录元数据、限制登录尝试并启用后台管理、黑白名单和内容审核。
...
| `POST /upload` | `functions/upload.js` | 先验证上传会话或有效后台 Basic Auth，再上传 Telegram、按需写 KV并返回文件路径数组 |
...
2. 前端向 `POST /upload` 提交带 `file` 的 `FormData`；通过认证后，后端确认 Telegram 配置、非空文件和大小限制。
...
7. 若绑定 `img_url`，保存元数据；KV 写入失败只记录安全警告，不把已完成的 Telegram 上传误报为失败。
...
使用 Node.js 22 和锁文件执行 `npm ci`，以 `npm start` 启动本地 Pages 环境。`npm test` 执行 Node 内置测试，`npm run ci-test` 同时启动 Wrangler 并运行测试。本地命令自带测试用后台与上传认证绑定，不得用于生产。

Cloudflare Pages 部署无需前端构建命令，仓库根目录作为静态资源目录，`functions/` 由 Pages 自动识别。生产上传前必须配置 Telegram 两项变量、上传密码和会话密钥，并绑定 `UPLOAD_AUTH_KV`；后台功能还需绑定 `img_url`。详细步骤见 `docs/DEPLOYMENT.md`。
