...
T-IMG 是一个静态前端与 Cloudflare Pages Functions 组合的文件托管项目。当前版本通过 Telegram Bot API 上传和存储文件，并为新上传自动生成同域短链；上传页面和上传接口由独立密码会话保护；必需的 `img_url` KV 保存短码映射、图片索引和元数据，独立 KV 限制登录尝试并支持后台管理、黑白名单和内容审核。
...
- 存储：Telegram 负责文件内容；Cloudflare KV 绑定 `img_url` 负责短码、完整 Telegram 文件标识、索引和元数据，独立绑定 `UPLOAD_AUTH_KV` 负责上传登录失败计数。
