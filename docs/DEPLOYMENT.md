...
5. Pages 读取根目录 `_redirects` 和 `_routes.json`；不得删除上传页面、登录页面及 Clean URLs 的 Functions 路由规则。登录页安全响应头由根级 Function 设置。
...
`img_url` 不是普通环境变量，而是生产上传必需的 KV Namespace 绑定。代码在向 Telegram 发送文件前检查它是否提供 KV `put()` 方法；未绑定、名称错误或误设为文本时，`POST /upload` 返回 503 且不会联系 Telegram。已存在的 `/file/:id` 仍保持公开访问；管理 API 会提示后台已禁用。Telegram 已接受文件后若发生瞬时 KV 写入失败，接口仍返回文件地址并记录不含敏感值的警告，避免用户重试造成 Telegram 重复文件。
...
- 验证错误密码、正确密码、刷新保持、退出、会话过期和直接 `POST /upload`；同时确认 `/file/:id` 公开访问及后台管理未被上传认证重定向。
